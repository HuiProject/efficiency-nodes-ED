...
def load_lora(lora_params, ckpt_name, id, cache=None, ckpt_cache=None, cache_overwrite=False,
    ...
    # 传入外部 MODEL/CLIP 时，缓存对象可能绑定了另一份基础模型，不能复用。
    for entry in loaded_objects["lora"] if model is None else []:
    ...
    def recursive_load_lora(lora_params, ckpt, clip, id, ckpt_cache, cache_overwrite, folder_paths):
        ...
        try:
            if not os.path.isabs(lora_name):
                # 与 ComfyUI 原生 LoRA Loader、Power Loader 使用完全相同的加载路径。
                lora_model, lora_clip = LoraLoader().load_lora(
            ...
            print(f"[XY-DEBUG] applied via ComfyUI LoraLoader: {lora_name} "
    ...
    # Anima/Qwen 的外部模型没有 checkpoint 文件名，直接使用当前 MODEL/CLIP。
